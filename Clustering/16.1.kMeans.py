...
if __name__ == "__main__":
    N = 400
    centers = 4
    data, y = ds.make_blobs(N, n_features=2, centers=centers, random_state=2)  #原始数据
    data2, y2 = ds.make_blobs(N, n_features=2, centers=centers, cluster_std=(1,2.5,0.5,2), random_state=2)  #方差不一样的数据分布
    data3 = np.vstack((data[y == 0][:], data[y == 1][:50], data[y == 2][:20], data[y == 3][:5]))#不均衡数据
    #print data[y==0][:50].shape
    y3 = np.array([0] * 100 + [1] * 50 + [2] * 20 + [3] * 5)#y3标签
