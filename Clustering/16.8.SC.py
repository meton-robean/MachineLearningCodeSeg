...
if __name__ == "__main__":
    ...
    data = np.vstack((data1, data2, data3))  # 3 圆环数据集合

    n_clusters = 3
    m = euclidean_distances(data, squared=True)  #距离矩阵
    sigma = np.median(m)

    plt.figure(figsize=(12, 8), facecolor='w')
    plt.suptitle(u'谱聚类', fontsize=20)
    clrs = plt.cm.Spectral(np.linspace(0, 0.8, n_clusters))  #给出几种颜色
