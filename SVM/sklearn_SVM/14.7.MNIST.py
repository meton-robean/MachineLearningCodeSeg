...
if __name__ == "__main__":
    classifier_type = 'SVM'  #可以选择使用多种分类方法 SVM RF。。
    ...
    #共显示32张图片
    for index, image in enumerate(images[:16]):#显示前16个训练图片
        plt.subplot(4, 8, index + 1)
        plt.imshow(image, cmap=plt.cm.gray_r, interpolation='nearest')
        plt.title(u'训练图片: %i' % y[index])
    for index, image in enumerate(images_test_result[:16]):#显示前16个测试图片
    ...
    if classifier_type == 'SVM':
        ...
        model.fit(x, y)
        t = time() - t
        print u'SVM训练结束，耗时%d分钟%.3f秒' % (int(t/60), t - 60*int(t/60))
        ...
        t = time() - t
        print u'SVM训练集准确率：%.3f%%，耗时%d分钟%.3f秒' % (accuracy_score(y, y_hat)*100, int(t/60), t - 60*int(t/60))
        t = time()
        y_test_hat = model.predict(x_test)
        t = time() - t
        print u'SVM测试集准确率：%.3f%%，耗时%d分钟%.3f秒' % (accuracy_score(y_test, y_test_hat)*100, int(t/60), t - 60*int(t/60))
        save_result(model)
