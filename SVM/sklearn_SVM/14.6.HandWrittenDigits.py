...
def save_image(im, i):
    im *= 15.9375
    im = 255 - im
    a = im.astype(np.uint8)  #float->uint8
...
if __name__ == "__main__":
    ...
    images = x.reshape(-1, 8, 8)  #向量，每个元素是8*8图像矩阵
    print images
    ...
    t1 = time()
    t = t1 - t0
    print u'训练+CV耗时：%d分钟%.3f秒' % (int(t/60), t - 60*int(t/60))
    # print '最优参数：\t', model.best_params_
    #clf.fit(x, y)
    print 'Learning is OK...'
    print u'训练集准确率：', accuracy_score(y, model.predict(x))
    y_hat = model.predict(x_test)
    print u'测试集准确率：', accuracy_score(y_test, model.predict(x_test))
    print y_hat
    print y_test
